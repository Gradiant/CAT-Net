...
class COCOannot(AbstractDataset):
    """
    directory structure
    ...
    def __init__(self, crop_size, grid_crop, blocks: list, DCT_channels: int, tamp_list: str, istrain: bool):
        ...
        if istrain:
            self._images_path = Path(project_config.dataset_paths['COCOannot_images_train'])
            self._masks_path = Path(project_config.dataset_paths['COCOannot_masks_train'])
        else:
            self._images_path = Path(project_config.dataset_paths['COCOannot_images_val'])
            self._masks_path = Path(project_config.dataset_paths['COCOannot_masks_val'])
        self._list_path = project_config.dataset_paths['COCOannot_list']
