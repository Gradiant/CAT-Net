...
from Splicing.data.AbstractDatasetCls import AbstractDatasetCls
...
class arbitraryCls(AbstractDatasetCls):
    ...
    def get_tamp(self, index):
        ...
        if im.format != "JPEG":
            ...
            tensors = self._create_tensor(temp_jpg, label)
            os.remove(temp_jpg)
        else:
            tensors = self._create_tensor(tamp_path, label)
        return tensors
